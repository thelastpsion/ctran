...
var
    ...
    ts : TStringList;
begin
    WriteLn('/* Generated by ctran from ', strFilename, ' */');
    WriteLn('#define ', UpCase(par.ModuleName), '_G');

    for inc_file in par.IncludeList do
    begin
        inc_file_def := UpCase(StringReplace(inc_file, '.', '_', [rfReplaceAll]));
        WriteLn('#ifndef ', inc_file_def);
        WriteLn('#include <', inc_file, '>');
        WriteLn('#endif');
    end;

    WriteLn('/* Category numbers */');
    WriteLn('#ifndef EPOC');
    WriteLn('GLREF_D P_CLASS *ct_', LowerCase(par.ModuleName), '[];');
    for i := 0 to length(par.ExternalList) - 1 do
    begin
        WriteLn('GLREF_D P_CLASS *ct_', par.ExternalList[i], '[];');
    end;
    WriteLn('#endif /* EPOC */');
    WriteLn('#ifdef EPOC');
    WriteLn('#define CAT_', par.ModuleName, '_', par.ModuleName, ' 0');
    for i := 0 to length(par.ExternalList) - 1 do
    begin;
        WriteLn('#define CAT_', par.ModuleName, '_', UpCase(par.ExternalList[i]), ' ', i + 1);
    end;
    WriteLn('#else');
    WriteLn('#define CAT_', par.ModuleName, '_', par.ModuleName, ' (@ct_', LowerCase(par.ModuleName), '[0])');
    for i := 0 to length(par.ExternalList) - 1 do
    begin;
        WriteLn('#define CAT_', par.ModuleName, '_', UpCase(par.ExternalList[i]), ' (@ct_', LowerCase(par.ExternalList[i]), '[', i + 1, '])');
    end;
    WriteLn('#endif');

    WriteLn('/* Class Numbers */');
    for i := 0 to length(par.ClassList) - 1 do
    begin
        WriteLn('#define C_', UpCase(par.ClassList[i].Name), ' ', i);
    end;

    WriteLn('/* Method Numbers */');
    method_id := 1;
        ...
        begin
            case method_item.MethodType of
                methodAdd, methodDefer: begin
                    WriteLn('#define O_', UpCase(method_item.Name), ' ', method_id);
                    Inc(method_id);
                end;
            end;
        end;
    ...
    begin
        if length(class_item.ClassConstants) > 0 then begin
            WriteLn('/* Constants for ', class_item.Name, ' */');
            for constant_item in class_item.ClassConstants do
            begin
                WriteLn('#define ', constant_item.Name, ' ', constant_item.Value);
            end;
        end;

        if length(class_item.ClassTypes) > 0 then begin
            WriteLn('/* Types for ', class_item.Name, ' */');
        ...
        WriteLn('/* Property of ', class_item.Name, ' */');
        if length(class_item.ClassProperty) > 0 then begin
            ...
            WriteLn('} PRS_', UpCase(class_item.Name), ';');
        end;
        WriteLn('typedef struct pr_', class_item.Name);
        WriteLn('{');
        ts := GetAncestorsWithProperty(class_item);
        for i := ts.Count - 1 downto 0 do
        begin
            WriteLn('PRS_', UpCase(ts[i]), ' ', ts[i], ';');
        end;
        if length(class_item.ClassProperty) > 0 then begin
            WriteLn('PRS_', UpCase(class_item.Name), ' ', class_item.Name, ';');
        end;
        WriteLn('} PR_', UpCase(class_item.Name), ';');
    end;
